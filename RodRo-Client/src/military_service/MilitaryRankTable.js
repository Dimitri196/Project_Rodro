...
const MilitaryRankTable = ({ items, deleteRank }) => {
    ...
    const filteredItems = useMemo(() => {
        const filtered = items.filter(item => {
            // Get the army branch name from the nested militaryOrganization
            const militaryOrganizationBranchName = item.militaryOrganization?.armyBranch?.armyBranchName || "";
            // New: Get the rank level name from the enum object, if it exists
            const rankLevelName = item.rankLevel?.name || "";

            return (
                normalizeString(item.rankName).includes(normalizeString(searchTerm)) ||
                normalizeString(rankLevelName).includes(normalizeString(searchTerm)) || // New: Filter by rank level name
                normalizeString(militaryOrganizationBranchName).includes(normalizeString(searchTerm))
            );
        });
                ...
                <Card.Body>
                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                        <InputGroup className="flex-grow-1">
                            <Form.Control
                                type="text"
                                placeholder="Search ranks by name, level, or organization/branch..." // Updated placeholder
                                value={searchTerm}
                                onChange={(e) => {
                        ...
                        <table className="table table-hover table-striped mb-0">
                            <thead className="bg-light">
                                <tr>
                                    ...
                                    <th className="py-3 px-4">Military Organization</th>
                                    <th className="py-3 px-4">Army Branch</th>
                                ...
                                {currentItems.length > 0 ? (
                                    currentItems.map((item, index) => (
                                        <tr key={item._id}>
                                            ...
                                            <td className="py-2 px-4">{item.rankLevel?.name || "-"}</td> {/* UPDATED LINE */}
                                            <td className="py-2 px-4">
                                                {item.militaryOrganization?.armyName || "-"}
                                            </td>
                                            <td className="py-2 px-4">
                                                {item.militaryOrganization?.armyBranch?.armyBranchName || "-"}
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={isAdmin ? 10 : 9} className="text-center py-4 text-muted">
