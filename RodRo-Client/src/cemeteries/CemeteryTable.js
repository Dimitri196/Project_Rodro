...
import { useSession } from "../contexts/session"; // ✅ Add this line

const CemeteryTable = ({ label, items, deleteCemetery }) => {
    const { session } = useSession(); // ✅ Get session
    const isAdmin = session.data?.isAdmin === true; // ✅ Check admin role
    ...
    return (
        <div className="container my-4">
            <div className="mb-3 d-flex justify-content-between align-items-center">
                ...
                {isAdmin && (
                    <Link to="/cemeteries/create" className="btn btn-success">Create Cemetery</Link>
                )}
            </div>
                    ...
                    <tbody>
                        {currentItems.map((item, index) => (
                            <tr key={item._id}>
                                ...
                                <td>{item.cemeteryName}</td>
                                <td>{item.cemeteryLocation?.locationName || "Unknown"}</td>
                                <td>{item.cemeteryParish?.parishName || "Unknown"}</td>
                                <td>{item.description || "N/A"}</td>
                                <td>
                                    <div className="btn-group">
                                        <Link to={`/cemeteries/show/${item._id}`} className="btn btn-sm btn-info mx-1">View</Link>
                                        {isAdmin && (
                                            <>
                                                <Link to={`/cemeteries/edit/${item._id}`} className="btn btn-sm btn-warning mx-1">Update</Link>
                                                <button
                                                    onClick={() => {
